		...
		&& objectp->mCreateSelected)
	{
		if ( LLToolMgr::getInstance()->getCurrentTool() != LLToolPie::getInstance() )
		{
			...
			if (gConvertToSculpt)
			{
				...
				gConvertToSculpt = false;
				sculpt_params.setSculptTexture(LLUUID(SCULPT_DEFAULT_TEXTURE));
...
void LLViewerObjectList::renderObjectsForMap(LLNetMap &netmap)
{
	...
	//Avoid performance hits by using LLCachedControl... not.
	S32  phantom_opacity=gSavedSettings.getS32("MiniMapPhantomOpacity");
