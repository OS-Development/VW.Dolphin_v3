...
LLFloaterBuildOptions::LLFloaterBuildOptions(const LLSD& key)
  : LLFloater(key),
    mComboGridMode(NULL)
{
	mCommitCallbackRegistrar.add("Pivot.Reset",           boost::bind(&LLFloaterBuildOptions::resetPivotPoint,   this));
}
...
void LLFloaterBuildOptions::resetPivotPoint()
{
	gSavedSettings.setBOOL("FSBuildPrefs_PivotIsPercent", TRUE);
	gSavedSettings.setF32("FSBuildPrefs_PivotX", 50.0);
	gSavedSettings.setF32("FSBuildPrefs_PivotY", 50.0);
	gSavedSettings.setF32("FSBuildPrefs_PivotZ", 50.0);
}
