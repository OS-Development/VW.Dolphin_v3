...
{
	gSavedPerAccountSettings.getControl("_NACL_UseAO")->getCommitSignal()->connect(boost::bind(&AOEngine::onToggleAOControl, this));
}
...
void AOEngine::init()
{
	mEnabled=gSavedPerAccountSettings.getBOOL("_NACL_UseAO");
	enable(mEnabled);
}
...
{
	enable(gSavedPerAccountSettings.getBOOL("_NACL_UseAO"));
}
...
void AOEngine::Toggle()
{
	mEnabled=!mEnabled;
	gSavedPerAccountSettings.setBOOL("_NACL_UseAO", mEnabled);
	enable(mEnabled);
}

bool AOEngine::isRunning()
{
      return mEnabled;
}
...
LLUUID AOEngine::addSet(const std::string& name,BOOL reload)
{
	...
	BOOL wasProtected=gSavedPerAccountSettings.getBOOL("_NACL_ProtectAOFolders");
	gSavedPerAccountSettings.setBOOL("_NACL_ProtectAOFolders",FALSE);
	lldebugs << "adding set folder " << name << llendl;
	LLUUID newUUID=gInventory.createNewCategory(mAOFolder,LLFolderType::FT_NONE,name);
	gSavedPerAccountSettings.setBOOL("_NACL_ProtectAOFolders",wasProtected);
...
void AOEngine::tick()
{
	const LLUUID categoryID=gInventory.findCategoryByName(ROOT_DOLPHIN3_FOLDER);

	if(categoryID.isNull())
	{
		llwarns << "no " << ROOT_DOLPHIN3_FOLDER << " folder yet. Creating ..." << llendl;
		gInventory.createNewCategory(gInventory.getRootFolderID(),LLFolderType::FT_NONE,ROOT_DOLPHIN3_FOLDER);
		mAOFolder.setNull();
	}
	else
	{
