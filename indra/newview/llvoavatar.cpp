...
//MK
	// Protect from a potential crash
////	else if (isRoot())
...
//		llinfos << "Setting timestep to " << time_quantum * pixel_area_scale << llendl;
	...
	if (!(mIsSitting && getParent()))
	{
		...
		if (!isAnyAnimationSignaled(AGENT_NO_ROTATE_ANIMS, NUM_AGENT_NO_ROTATE_ANIMS))
		{
			...
			velDir.normalize();
			if (!gSavedSettings.getBOOL("_NACL_TurnAroundWhenWalkingBackwards") && (mSignaledAnimations.find(ANIM_AGENT_WALK) != mSignaledAnimations.end()))
			{
