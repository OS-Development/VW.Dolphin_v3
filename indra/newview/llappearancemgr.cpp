...
//	gAgentWearables.notifyLoadingStarted();
	...
	case LLAssetType::AT_CLOTHING:
		if (gAgentWearables.areWearablesLoaded())
		{
			...
			{
//MK
				if ((gRRenabled && gAgent.mRRInterface.canUnwear (item_to_wear->getWearableType())) || !gRRenabled)
				{
//mk
...
//MK
			if ((gRRenabled && gAgent.mRRInterface.canWear (item_to_wear->getWearableType())) || !gRRenabled)
			{
//mk
...
//MK
	if (gRRenabled)
	{
		...
		// Depending on the command (replace or add), we must retain what we chose in order to decide what to do with the list
		// of wearables once LLAgentWearables::userUpdateAttachments() is called
	}
