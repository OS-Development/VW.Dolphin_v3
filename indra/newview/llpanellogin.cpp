...
void LLPanelLogin::updateLocationCombo( bool force_visible )
{
 ...
 //MK
//LC 	if (gSavedSettings.getBOOL("RestrainedLove"))
//	{
//		combo->setCurrentByIndex( 0 );	
// 		show_start = FALSE;
//	}
...
void LLPanelLogin::loadLoginPage()
{
	...
	if(strcmp(curl_grid, curl_escape("Second Life",0))==0) curl_grid = curl_escape("Agni",0);
	if(strcmp(curl_grid, curl_escape("Second Life Beta",0))==0) curl_grid = curl_escape("Aditi",0);
	oStr << "&grid=" << curl_grid;
	curl_free(curl_grid);
