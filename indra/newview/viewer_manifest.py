...
#                  dst="updater.exe")
    ...
    def package_finish(self):
        ...
        if self.default_channel():
            if self.default_grid():
                # release viewer
                installer_file = "DolphinViewer3-i686-Windows-%(version_dots)s.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "DolphinViewer3"
                !define SHORTCUT   "Dolphin Viewer 3"
                !define URLNAME   "secondlife"
                Caption "Dolphin Viewer 3 ${VERSION}"
                """
            else:
                # beta grid viewer
                installer_file = "DolphinViewer3-i686-Windows-%(version_dots)s-(%(grid_caps)s).exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "DolphinViewer3%(grid_caps)s"
                !define SHORTCUT   "Dolphin Viewer 3 (%(grid_caps)s)"
                !define URLNAME   "dolphinviewer3%(grid)s"
                !define UNINSTALL_SETTINGS 1
                Caption "Dolphin Viewer 3 %(grid)s ${VERSION}"
                """
        else:
...
class DarwinManifest(ViewerManifest):
    ...
    def construct(self):
        ...
        self.path(self.args['configuration'] + "/Dolphin Viewer 3.app", dst="")
        ...
        if self.prefix(src="", dst="Contents"):  # everything goes in Contents
            ...
            if self.prefix(src="", dst="Resources"):
                ...
                if self.prefix(src=icon_path, dst="") :
                    self.path("dolphinviewer_icon.icns")
        ...
        if ("package" in self.args['actions'] or 
            ...
            self.run_command('strip -S %(viewer_binary)r' %
                             { 'viewer_binary' : self.dst_path_of('Contents/MacOS/Dolphin Viewer 3')})
    ...
    def package_finish(self):
        channel_standin = 'Dolphin Viewer 3'  # hah, our default channel is not usable on its own
        if not self.default_channel():
            channel_standin = self.channel()

        imagename="DolphinViewer3_" + '_'.join(self.args['version'])

        # MBW -- If the mounted volume name changes, it breaks the .DS_Store's background image and icon positioning.
        #  If we really need differently named volumes, we'll need to create multiple DS_Store file images, or use some other trick.

        volname="Dolphin Viewer 3 Installer"  # DO NOT CHANGE without understanding comment above
        ...
        try:
            ...
            if self.default_channel() and not self.default_grid():
                app_name = "Dolphin Viewer 3 " + self.args['grid']
            else:
...
class Linux_i686Manifest(LinuxManifest):
    def construct(self):
        ...
        if self.prefix("../packages/lib/release", dst="lib"):
            ...
            self.path("libfreetype.so.6.3.20")
