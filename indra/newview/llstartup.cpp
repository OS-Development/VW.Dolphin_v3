...
//mk
	...
	if ( STATE_FIRST == LLStartUp::getStartupState() )
	{
		...
		std::string checkurl = "http://dolphinviewer.eregion.de/updatecheck.php";
		...
		stream << LLVersionInfo::getChannel() << "-" << LLVersionInfo::getVersion();
		myUserAgent = stream.str();
		curl = curl_easy_init();
		std::string response;
                                
		if (curl) {
			curl_easy_setopt(curl, CURLOPT_URL, checkurl.c_str());
			curl_easy_setopt(curl, CURLOPT_USERAGENT, myUserAgent.c_str());
		...
		if(response == "0")
		{
			LL_INFOS("Update Check") << "You are not running the latest official release of the Dolphin Viewer." << LL_ENDL;
			LLNotificationsUtil::add("DolphinViewerUpdate");
		}
	...
	if (STATE_LOGIN_CLEANUP == LLStartUp::getStartupState())
	{
		...
		gDirUtilp->setLindenUserDir(LLGridManager::getInstance()->getGridLabel(), userid);
		...
		gDirUtilp->setPerAccountChatLogsDir(LLGridManager::getInstance()->getGridLabel(),userid);  
...
//MK
		...
		{
		   LL_INFOS("RestrainedLove") << "RLV Disabled or no TP restriction" << LL_ENDL ;
//mk
...
//MK/LC
		if (gRRenabled && !gSavedPerAccountSettings.getBOOL("RestrainedLoveTPOK"))
		{
			LL_INFOS("RestrainedLove") << "RLV Enabled and under TP restriction, logging in to " << LLSLURL::SIM_LOCATION_LAST << LL_ENDL ;
...
//mk/lc
	...
	if (STATE_INVENTORY_SEND == LLStartUp::getStartupState())
	{
		...
		std::string gWindowTitle = gDisplayName + " - " + LLVersionInfo::getChannel();
