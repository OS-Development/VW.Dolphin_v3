...
//	mCommitCallbackRegistrar.add("Pref.ClickResetDialogs",		boost::bind(&LLFloaterPreference::onClickResetDialogs, this));
	...
	mCommitCallbackRegistrar.add("Pref.CommitRLVEnabled", 		boost::bind(&LLFloaterPreference::onCommitRLVEnabled, this));

	mCommitCallbackRegistrar.add("Pref.CommitSnapshot2DiskFormatChanged", boost::bind(&LLFloaterPreference::onCommitSnapshot2DiskFormat , this));
...
BOOL LLFloaterPreference::postBuild()
{
	...
	// if floater is opened before login, disable RLV-related checkboxes
	if (LLStartUp::getStartupState() != STATE_STARTED)
	{
		getChild<LLUICtrl>("restrained_love")->setEnabled(FALSE);
		getChild<LLUICtrl>("restrained_love_no_set_env")->setEnabled(FALSE);
		getChild<LLUICtrl>("restrained_love_forbid_give_to_rlv")->setEnabled(FALSE);
		getChild<LLUICtrl>("restrained_love_allow_wear")->setEnabled(FALSE);
		getChild<LLUICtrl>("rlv_reattach_delay")->setEnabled(FALSE);
	} else
	{
		getChild<LLUICtrl>("restrained_love")->setEnabled(TRUE);
		getChild<LLUICtrl>("restrained_love_no_set_env")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
		getChild<LLUICtrl>("restrained_love_forbid_give_to_rlv")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
		getChild<LLUICtrl>("restrained_love_allow_wear")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
		getChild<LLUICtrl>("rlv_reattach_delay")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
	}

	getChild<LLUICtrl>("fetch_inventory_on_login")->setEnabled(!(gSavedSettings.getBOOL("RestrainedLove")));
	if(gSavedSettings.getBOOL("RestrainedLove"))
	{
	  gSavedSettings.setBOOL("FetchInventoryOnLogin", TRUE);
	}

	getChild<LLUICtrl>("snapshot2disk_jpg_quality")->setEnabled(gSavedSettings.getS32("Snapshot2DiskFormat")==1);
	getChild<LLComboBox>("snapshot2disk_format_combo")->selectNthItem((S32) gSavedSettings.getS32("Snapshot2DiskFormat"));
	return TRUE;
}
...
void LLFloaterPreference::onCommitRLVEnabled()
{
	getChild<LLUICtrl>("restrained_love_no_set_env")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
	getChild<LLUICtrl>("restrained_love_forbid_give_to_rlv")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
	getChild<LLUICtrl>("restrained_love_allow_wear")->setEnabled(gSavedSettings.getBOOL("RestrainedLove"));
...
void LLFloaterPreference::onCommitSnapshot2DiskFormat()
{
	LLComboBox* local_format_combo = getChild<LLComboBox>("snapshot2disk_format_combo");
	const std::string id  = local_format_combo->getValue().asString();
	if (id == "PNG")
	{
		gSavedSettings.setS32("Snapshot2DiskFormat", 0);;
	}
	else if (id == "JPEG")
	{
		gSavedSettings.setS32("Snapshot2DiskFormat", 1);;
	}
	else if (id == "BMP")
	{
		gSavedSettings.setS32("Snapshot2DiskFormat", 2);;
	}
	getChild<LLUICtrl>("snapshot2disk_jpg_quality")->setEnabled(gSavedSettings.getS32("Snapshot2DiskFormat")==1);
	refresh();
}
