...
:	LLFloater(key),LLEventTimer(10.0),
	mSetList(0),
	mSelectedSet(0),
	mSelectedState(0),
...
BOOL FloaterAO::postBuild()
{
	...
	// NaCl - Make animation directly playable
	mAnimationList->setDoubleClickCallback(boost::bind(&FloaterAO::onDoubleClickAnimation,this));
	// NaCl End
	...
	if(gSavedPerAccountSettings.getBOOL("_NACL_UseFullAOInterface"))
		onClickMore();
	else
		onClickLess();
...
{
	LLRect fullSize=gSavedPerAccountSettings.getRect("_NACL_floater_rect_animation_overrider_full");
	...
	gSavedPerAccountSettings.setBOOL("_NACL_UseFullAOInterface",TRUE);
...
void FloaterAO::onClickLess()
{
	...
	gSavedPerAccountSettings.setRect("_NACL_floater_rect_animation_overrider_full",fullSize);
	...
	gSavedPerAccountSettings.setBOOL("_NACL_UseFullAOInterface",FALSE);
...
void FloaterAO::onDoubleClickAnimation()
{
	S32 currentIndex=mAnimationList->getFirstSelectedIndex();
	LLScrollListItem* lSelected=mAnimationList->getFirstSelected();
	S32 currentStateSelected=mStateSelector->getCurrentIndex();
	AOSet::AOState* state=mSelectedSet->getState(currentStateSelected);
	state->mCurrentAnimation=currentIndex;
}
