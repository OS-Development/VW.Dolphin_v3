...
Icon          %%SOURCE%%\installers\windows\dolphin_install_icon.ico
UninstallIcon %%SOURCE%%\installers\windows\dolphin_uninstall_icon.ico
...
Function CloseSecondLife
  Push $0
  FindWindow $0 "The Dolphin Viewer 3" ""
  IntCmp $0 0 DONE
...
Function CheckWillUninstallV2
  ...
  StrCmp $INSTDIR "$PROGRAMFILES\DolphinViewer3" CHECKV2_DONE ; don't uninstall our own install dir.
  IfFileExists "$PROGRAMFILES\DolphinViewer3\uninst.exe" CHECKV2_FOUND CHECKV2_DONE
...
WriteINIStr		"$SMPROGRAMS\$INSTSHORTCUT\Dolphin Viewer Homepage.url" \
				"InternetShortcut" "URL" \
				"http://dolphinviewer.eregion.de/"
WriteINIStr		"$SMPROGRAMS\$INSTSHORTCUT\The Dolphin Viewer Forum.url" \
				"InternetShortcut" "URL" \
				"http://dolphinviewer.eregion.de/forum/"
...
StrCmp $DO_UNINSTALL_V2 "" REMOVE_SLV2_DONE
  ExecWait '"$PROGRAMFILES\DolphinViewer3\uninst.exe" /S _?=$PROGRAMFILES\SecondLifeViewer2'
  Delete "$PROGRAMFILES\DolphinViewer3\uninst.exe" ; with _? option above, uninst.exe will be left behind.
  RMDir "$PROGRAMFILES\DolphinViewer3" ; will remove only if empty.
