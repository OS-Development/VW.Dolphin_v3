...
void RRInterface::refreshTPflag(bool save)
{
        if (gAgent.mRRInterface.mContainsUnsit || gAgent.mRRInterface.contains("tplm") || gAgent.mRRInterface.contains("tploc")) {
                gSavedPerAccountSettings.setBOOL("RestrainedLoveTPOK", FALSE);
        } else {
                gSavedPerAccountSettings.setBOOL("RestrainedLoveTPOK", TRUE);
        }
        if (save) {
...
//					gAgentWearables.removeWearable (type, false, 0); // remove by layer
	...
	//}
        ...
        if (!gSavedSettings.controlExists("RestrainedLoveOffsetAvatarZ")) {
            return FALSE;
        }
        LLVOAvatar* avatar = gAgentAvatarp;
        if (avatar) {
            ...
            gSavedSettings.setF32("RestrainedLoveOffsetAvatarZ", val);
        }
	}
