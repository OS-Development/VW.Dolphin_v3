...
std::string LLDir::buildSLOSCacheDir() const
{
	...
	if (getOSCacheDir().empty())
	{
		...
		{
			res = getOSUserAppDir() + mDirDelimiter + "dv3cache";
		}
	}
	else
	{
		res = getOSCacheDir() + mDirDelimiter + "DolphinViewer3";
	}
	return res;
}
...
void LLDir::setLindenUserDir(const std::string &grid, const std::string &username)
{
	...
	if (!username.empty())
	{
		...
		// Append name of grid but only when not in SL
		if(!grid.empty() && gridlower.find("second life") == std::string::npos)
		{
...
void LLDir::setPerAccountChatLogsDir(const std::string &grid, const std::string &username)
{
